...
@csrf_exempt
@login_required
def inventory_hosts_view(request, inventory_id):
    try:
        status, msg, inv = InventoryAPI.get(request.user, inventory_id)
        ...
        context = {
            ...
            'path_api_inventory_hosts': reverse('inventory:api_inventory_hosts', kwargs={'inventory_id': inv.id}),
            'path_api_host_create': reverse('inventory:api_host_create')
        }

        return render(request, 'inventory/inventory_hosts.html', context)
    except Exception as e:
        return HttpResponse(str(e))
...
@csrf_exempt
@login_required
def inventory_groups_view(request, inventory_id):
    try:
        status, msg, inv = InventoryAPI.get(request.user, inventory_id)
        ...
        context = {
            ...
            'path_api_inventory_groups': reverse('inventory:api_inventory_groups', kwargs={'inventory_id': inv.id}),
            'path_api_group_create': reverse('inventory:api_group_create')
        }

        return render(request, 'inventory/inventory_groups.html', context)
    except Exception as e:
        return HttpResponse(str(e))
...
@csrf_exempt
@login_required
def inventory_group_detail_view(request, inventory_id, group_id):
    try:
        status, msg, inv = InventoryAPI.get(request.user, inventory_id)
        ...
        context = {
            ...
            'group': group,
            'hosts': inv.host_set.all(),
            'groups': inv.group_set.all(),
            'path_api_edit': reverse('inventory:api_group_edit', kwargs={'group_id': group.id})
        }

        return render(request, 'inventory/inventory_group_detail.html', context)
    except Exception as e:
        return HttpResponse(str(e))
        ...
        if request.is_ajax() and request.method == 'GET':
            ...
            for i in inv.host_set.all():
                hosts.append({
                    ...
                    'path_detail': reverse('inventory:inventory_host_detail', kwargs={'inventory_id': inv.id, 'host_id': i.id}),
        ...
        if request.is_ajax() and request.method == 'GET':
            ...
            for i in inv.group_set.all():
                groups.append({
                    ...
                    'path_detail': reverse('inventory:inventory_group_detail', kwargs={'inventory_id': inv.id, 'group_id': i.id}),
...
@csrf_exempt
@login_required
def api_group_edit_view(request, group_id):
    try:
        if request.is_ajax and request.method == 'POST':
            data = request.POST.dict()
            status, msg = GroupAPI.update(
                request.user, group_id, **data
            )
...
@csrf_exempt
@login_required
def api_group_delete_view(request):
    try:
        if request.is_ajax and request.method == 'POST':
            data = request.POST.dict()
            status, msg = GroupAPI.delete(request.user, data.get('group_id'))
...
@csrf_exempt
@login_required
def api_add_host_into_group_view(request):
    try:
        if request.method == 'POST':
            data = request.POST
            host_id_list = data.getlist('host_id')
...
@csrf_exempt
@login_required
def api_remove_host_from_group_view(request):
    try:
        if request.method == 'POST':
            data = request.POST.dict()
...
@csrf_exempt
@login_required
def api_add_group_into_group_view(request):
    try:
        if request.method == 'POST':
            data = request.POST
            cgid_list = data.getlist('cgid')
...
@csrf_exempt
@login_required
def api_remove_group_from_group_view(request):
    try:
        if request.method == 'POST':
            data = request.POST.dict()
